...
type KnownLiteralTypes<T extends { type: string }> = T extends { type: infer U }
  ? string extends U
    ? never
    : U
  : never;
...
export type IncomingMessage<T extends MessageBase> = Partial<T> & {
...
export type SystemOutboundMessage = MessageBase<'error'> & { message: string };
